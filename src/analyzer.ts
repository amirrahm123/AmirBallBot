...
- אם אין מספיק פריימים ברצף — תאר מה שאתה רואה בוודאות, גם אם זה קצר
- אם אתה רואה פחות מ-3 פריימים ברצף של אותו מהלך — תאר מה שאתה רואה בוודאות, אל תמציא
...
═══════════════════════════════════════
PLAY IDENTIFICATION GUIDE — Visual Rules Per Play Type
═══════════════════════════════════════

חטיפה (Steal):
פריימים: הגנה צמודה → מגע יד על כדור → כדור עובר לשליטת ההגנה → מתפרצת
חייב: מגע הגנתי ישיר + מעבר שליטה להגנה. הקרדיט לשחקן שנגע ראשון, לא למי שתפס
לא חטיפה: כדור נפל בלי מגע הגנתי / שחקן מרים כדור חופשי ללא deflection
טעות נפוצה: לתת קרדיט חטיפה כשההתקפה פשוט עשתה טעות

אלי אופ (Alley-Oop):
פריימים: שחקן רץ לסל ללא כדור → מסירת lob גבוהה → קפיצה ולכידה מעל הרינג → סיום בתנועה רציפה
חייב: מסירה מכוונת + סיום רציף מהאוויר ללא נחיתה בין לכידה לסיום
לא אלי אופ: כדור מהלוח / שחקן נחת והתייצב לפני הסיום / מסירה שהפכה לריבאונד
טעות נפוצה: בלבול עם ריבאונד התקפי + פוטבק

בלוק (Block):
פריימים: תוקף עולה לזריקה → מגן מגיב → יד על כדור בזמן תנועת הזריקה → כדור משנה כיוון
חייב: מגע בכדור בזמן upward/release phase של הזריקה
לא בלוק: מגע לפני תחילת תנועת זריקה (= חטיפה) / פאול בזריקה
טעות נפוצה: בלבול בין בלוק לחטיפה — ההבדל הוא תזמון המגע

הנעת כדור (Ball Movement):
פריימים: כדור בצד אחד → 2+ פעולות (מסירה/חדירה) → תזוזת הגנה → שחקן פנוי → יתרון
חייב: לפחות 2 פעולות שמזזות לפחות 2 שחקני הגנה + יצירת יתרון התקפי
לא הנעה: מסירה בודדת / כדור עומד יותר מ-3 שניות / מסירות שלא משנות מבנה הגנה
טעות נפוצה: לתייג כל מסירה כהנעת כדור

איבוד כדור (Turnover):
פריימים: שחקן עם כדור → רגע איבוד → כדור חופשי/אצל יריב → שליטה הגנתית
חייב: אובדן שליטה התקפית + מעבר שליטה להגנה
סוגים: bad pass / ball handling error
לא איבוד: בלוק שיצא חוץ ללא שליטה הגנתית / ריבאונד הגנתי רגיל
הבהרה: בלוק שההגנה משתלטת עליו = כן איבוד
טעות נפוצה: לא מבדיל איבוד עצמי מחטיפה

iso (Isolation):
פריימים: שחקן שולט בכדור ≥2 שניות → אין פעולה קבוצתית → יצירת יתרון אישי → זריקה/חדירה
חייב: אין screen/cut אקטיבי + שליטה של שחקן אחד + כוונה ליתרון אישי
לא iso: חדירה מקרית / פיק אנד רול / תנועה קבוצתית
טעות נפוצה: לתייג כל 1v1 כ-iso

פיק אנד רול (Pick & Roll):
פריימים: הצבת מסך על שומר הכדור → שחקן עם כדור משתמש במסך → יתרון → המשך התקפה
חייב: מסך ברור על שומר הכדור + שימוש במסך (שינוי כיוון/מרחק)
כולל: reject screen (שימוש מדומה)
לא P&R: מסך ללא שימוש / שחקן לא עובר ליד המסך
טעות נפוצה: בלבול עם iso כשהמסך לא נראה בבירור

ריבאונד (Rebound):
פריימים: ניסיון זריקה → פגיעה ברינג/לוח/החטאה → מאבק על כדור → שליטה חדשה
חייב: ניסיון זריקה קודם + איבוד שליטה + שליטה חדשה
כולל: tip rebound / free throw rebound
לא ריבאונד: חטיפה / אלי אופ / כדור חופשי ללא זריקה קודמת
טעות נפוצה: לתייג כל קפיצה לכדור גבוה כריבאונד

hustle plays:
פריימים: כדור לא בשליטה → תגובת שחקן → מאמץ פיזי חריג → הצלה/שליטה
חייב: כדור לא בשליטה + מאמץ פיזי חריג (צלילה/נפילה/מתיחה קיצונית) + תוצאה חיובית
לא hustle: ריבאונד רגיל / חטיפה קלה / ריצה רגילה
טעות נפוצה: לא לזהות כי זה לא מהלך "יפה" — זה מאמץ

שלשה (3PT Shot):
פריימים: שחקן מאחורי קו השלש → עלייה → שחרור מאחורי הקו → כדור באוויר → תוצאה
חייב: רגע שחרור הכדור מאחורי קו השלש
הבהרה: קפיצה מאחורי הקו ונחיתה קדימה = עדיין שלשה
לא שלשה: רגל על הקו בזמן השחרור / זריקה מטווח 2
טעות נפוצה: לא להבדיל שלשה פתוחה (catch & shoot) מ-off-dribble — שתיהן שלשה`;
