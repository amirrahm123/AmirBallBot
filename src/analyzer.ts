...
function buildSystemPrompt(roster?: RosterPlayer[], teamName?: string): string {
  let prompt = SYSTEM_PROMPT;
  if (roster && roster.length > 0) {
    const team = teamName || 'הקבוצה שלנו';
    const rosterText = roster.map(p => `#${p.number} ${p.name} - ${p.position}`).join('\n');
    prompt += `\n\n===== מיקוד בקבוצה =====
הרוסטר שלהלן הוא של הקבוצה שלנו: ${team}.
רכז את הניתוח רק על המהלכים וההחלטות של הקבוצה הזו.
אל תנתח את הטקטיקה של היריב בפירוט.
כשאתה מציין בעיות הגנתיות, התמקד במה שהקבוצה שלנו עשתה לא נכון — לא במה שהיריב עשה נכון.
...
export async function analyzeYouTubeCloud(url: string, context: string, focus: string, roster?: RosterPlayer[], teamName?: string): Promise<AnalysisResult> {
  ...
  const response = await client.messages.create({
    ...
    max_tokens: 8192,
    system: buildSystemPrompt(roster, teamName),
...
export async function analyzeFrames(frames: string[], context: string, focus: string, roster?: RosterPlayer[], teamName?: string): Promise<AnalysisResult> {
...
export async function analyzeGoogleDrive(url: string, context: string, focus: string, roster?: RosterPlayer[], teamName?: string): Promise<AnalysisResult> {
  ...
  const result = await analyzeFrames(frames, context, focus, roster, teamName);
...
export async function analyzeYouTube(url: string, context: string, focus: string, roster?: RosterPlayer[], teamName?: string): Promise<AnalysisResult> {
...
export async function analyzeVideo(videoPath: string, context: string, focus: string, roster?: RosterPlayer[], teamName?: string): Promise<AnalysisResult> {
...
export async function analyzeImage(imagePath: string, context: string, focus: string, roster?: RosterPlayer[], teamName?: string): Promise<AnalysisResult> {
  console.log('\n🖼️ Analyzing single image...');
  return analyzeFrames([imagePath], context, focus, roster, teamName);
}
