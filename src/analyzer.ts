...
כלל חשוב בכדורסל:
- קבוצת הבית לובשת תמיד חולצות בצבע בהיר (לבן, צהוב בהיר, אפור בהיר)
- קבוצת החוץ לובשת תמיד חולצות בצבע כהה (אדום, שחור, כחול, צבעים כהים)

השתמש בכלל הזה כדי לזהות קבוצות באופן אוטומטי בכל פריים:
- שחקנים בחולצות בהירות = קבוצת הבית (הקבוצה שלנו מהרוסטר)
- שחקנים בחולצות כהות = קבוצת האורחים (היריב)
...
function buildSystemPrompt(roster?: RosterPlayer[], teamName?: string, awayTeam?: string): string {
  ...
  if (roster && roster.length > 0) {
    ...
    prompt += `\n\n===== מיקוד בקבוצה =====
קבוצת בית (חולצות בהירות): ${home}
קבוצת חוץ (חולצות כהות): ${away}

הרוסטר שלהלן הוא של קבוצת הבית: ${home}.
רכז את הניתוח רק על המהלכים וההחלטות של ${home}.
אל תנתח את הטקטיקה של ${away} בפירוט.
כשאתה מציין בעיות הגנתיות, התמקד במה ש-${home} עשתה לא נכון — לא במה ש-${away} עשה נכון.
